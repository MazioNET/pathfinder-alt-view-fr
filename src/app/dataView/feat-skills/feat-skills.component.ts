...
@Component({
  selector: 'app-feat-skills',
  standalone: true,
  imports: [NgForOf, NgIf, NgClass, FormsModule],
  templateUrl: './feat-skills.component.html',
  styleUrl: './feat-skills.component.less'
})
export class FeatSkillsComponent {
  ...
  isElementDisplayed(element) : boolean{
    if(element["category"] != 'skill') return false;
  ...
  dynamicFilter(){
    ...
    this.data.forEach( element => {
      ...
      element["grey"] = grey;
      grey = !grey;
      if( this.textFilter != "" && element["translations"]["fr"]["name"] && !element["translations"]["fr"]["name"].toString().normalize('NFD').replaceAll(/[\u0300-\u036f]/g, '').toUpperCase()
