...
@Component({
  selector: 'app-feat-class',
  standalone: true,
  imports: [NgForOf, NgIf, NgClass, FormsModule],
  templateUrl: './feat-class.component.html',
  styleUrl: './feat-class.component.less'
})
export class FeatClassComponent {
        ...
        if(this.isElementDisplayed(element)){
          ...
          element["traits"]["value"].forEach( t =>{
            this.ancestryData.forEach(a =>{
              if(a['name'] == t['name']) element["ancestry"].push(a['translations']['fr']['name'].split(' ou ')[0]);
            });
          });
  ...
  isElementDisplayed(element) : boolean{
    if(element["category"] != 'class') return false;
